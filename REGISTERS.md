...
In addition to being compatible with a genuine VIC-II, VIC-II Kawari
...
software from unintentionally triggering extra registers.  NOTE: If
lock bit jumper #2 is removed, extra registers cannot be activated.
...
may be used to access VIC-II Kawari extra features. Extra regsters
...
VIC-II Kawari adds 32k of video memory to the C64. This memory can be directly
accessed by VIC-II Kawari for new graphics modes.  The registers 0xd039-0xd03f
...
of video RAM is mapped to extra registers for special VIC-II Kawari
functions.

### RGB Color Registers (For DVI/VGA)

VIC-II Kawari has a configurable color palette. The 16 colors can be selected
...
Colors generated by the luma/chroma generator can also be modified by changing registes 0xa0-0xaf (luma levels), 0xb0-0xbf (hue/phase angles) and 0xc0-0xcf (saturation/amplitude).

### All registers

Location | Name | Description | Capability Requirement | Restored After Boot? 
---------|------|-------------|------------------------|---------------------
...
0x82 | CHIP_MODEL | Chip Model Select (0=6567R8, 1=6569, 2=6567R56A) | NONE | Y
0x83 | VERSION | Version (high nibble major, low nibble minor) - Read Only | NONE | N/A
0x84 | DISPLAY_FLAGS | See below | NONE | Y
...
VIC-II Kawari variants. This way, the main branch configuration utility will
...
they use the main branch config utility.  Also, programs can run a simple check
